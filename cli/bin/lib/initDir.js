...
export default function initDir(){
    ...
    find(Directories.access, path=>{if(fs.readFileSync(path, "utf-8") === "" && path.endsWith(Files.extname.access))Models.rw.access=path});

    if(!fs.existsSync(Directories.checker))fs.mkdirSync(Directories.checker);
    find(Directories.checker, path=>{if(fs.readFileSync(path, "utf-8") === "" && path.endsWith(Files.extname.checker))Models.rw.checker=path});

    if(!fs.existsSync(Directories.register))fs.mkdirSync(Directories.register);
    find(Directories.register, path=>{if(fs.readFileSync(path, "utf-8") === "" && path.endsWith(Files.extname.rigister))Models.rw.register=path});

    if(!fs.existsSync(Directories.method))fs.mkdirSync(Directories.method);
    find(Directories.method, path=>{if(fs.readFileSync(path, "utf-8") === "" && path.endsWith(Files.extname.method))Models.rw.method=path});
}